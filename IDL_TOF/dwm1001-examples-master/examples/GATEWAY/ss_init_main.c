...
static uint8 tx_gateway_msg[] = {
    0x99, 0x99, // frame control // 0-1
    0x00,// BS NUMBER // 2
    0x00, 0x00, // distance // 3-4
    0x99, 0x99, // frame control // 5-6
    0x00, 0x00 // ranging bytes // 7-8
};
...
#define RX_BUF_LEN 13
static uint8 rx_buffer[RX_BUF_LEN];
static uint8 rx_buffer_last[RX_BUF_LEN];
...
static void print_msg(uint8 *msg, uint32 size);

/*Transactions Counters */
static volatile int tx_count = 0 ; // Successful transmit counter
static volatile int rx_count = 0 ; // Successful receive counter 

static uint16 distances[3] = {0, 0, 0};

int ss_init_run(void)
{
    dwt_forcetrxoff(); // temiz bir Rx icin anten her turlu moda kapatilir
    dwt_rxreset(); 
    dwt_rxenable(DWT_START_RX_IMMEDIATE); // RX start
    dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXFCG); // sistem status id registeri RX islemi icin ayarlanir
    dwt_readrxdata(rx_buffer, RX_BUF_LEN,0); // RX data Rx registerlerindan okunur
    
    
    //print_msg(rx_buffer, sizeof(rx_buffer));
    ...
    {
        if(rx_buffer[i] != rx_buffer_last[i])
        {
            same = false;
        }
    }
    ...
    if(!same)
    {
        for(i = 0; i<RX_BUF_LEN; i++)
        {
            rx_buffer_last[i] = rx_buffer[i];
        }
        if(rx_buffer[0] == 0x99) // 0x99 gelen mesajin bir anchordan geldigini ve bu mesajin bir TOF sonucu elde edilen (Anchor-Tag arasi) uzaklik bilgisini sakladigi bilinir
        {
            distances[0] = (uint16)rx_buffer[2]<<8 | (uint16)rx_buffer[3];
            distances[1] = (uint16)rx_buffer[4]<<8 | (uint16)rx_buffer[5];
            distances[2] = (uint16)rx_buffer[6]<<8 | (uint16)rx_buffer[7];
            
            printf("%04u%04u%04u\r\n", distances[0], distances[1], distances[2]);
            
            //print_msg(rx_buffer, sizeof(rx_buffer));//print_msg(rx_buffer, sizeof(rx_buffer));
        }
